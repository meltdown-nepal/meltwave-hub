...
const TestimonialsSection = ({
  ...
  return <section id="testimonials" className="py-16 px-4 md:px-8 bg-gradient-to-br from-yellow-50 via-white to-amber-50/40 overflow-hidden bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {testimonials.map((testimonial, idx) => <Card key={idx} className="group bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-500 border-0 h-full transform hover:-translate-y-1 relative overflow-visible">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="relative">
                      {testimonial.image ? <img src={testimonial.image} alt={testimonial.name} className="h-10 w-10 rounded-full object-cover shadow-lg border-2 border-primary bg-white" /> : <div className="h-10 w-10 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-black font-bold text-lg shadow-lg">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {testimonial.role && <p className="text-xs text-gray-600 leading-tight mt-0.5">{testimonial.role}</p>}
                    </div>
                  </div>
                  {/* Watch Button */}
                  {testimonial.linkTo && <Button variant="outline" size="sm" className="flex items-center gap-1.5 border-2 border-primary/20 hover:border-primary hover:bg-primary/10 text-gray-700 hover:text-black rounded-full px-3 py-1.5 text-xs font-medium group/btn transition-all duration-300" asChild>
        ...
        <div className="md:hidden w-full">
          ...
          loop: true
            ...
            <CarouselContent>
              {testimonials.map((testimonial, idx) => <CarouselItem key={idx} className="pl-2">
                    ...
                    <CardContent className="p-6 relative h-full flex flex-col min-h-[280px]">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          {testimonial.image ? <img src={testimonial.image} alt={testimonial.name} className="h-9 w-9 rounded-full object-cover shadow-lg border-2 border-primary bg-white" /> : <div className="h-9 w-9 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-black font-bold text-lg shadow-lg">
                              {testimonial.name?.charAt(0)}
                            </div>}
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-gray-900 text-sm truncate">{testimonial.name}</p>
                        ...
                        {testimonial.linkTo && <Button variant="outline" size="sm" className="flex items-center gap-1.5 border-2 border-primary/20 hover:border-primary hover:bg-primary/10 text-gray-700 hover:text-black rounded-full px-3 py-1 text-xs font-medium group/btn transition-all duration-300 ml-1" asChild>
                            <Link to={testimonial.linkTo} target="_blank" rel="noopener noreferrer">
                              <div className="w-5 h-5 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center group-hover/btn:scale-110 transition-transform duration-300">
                                <Play className="w-2.5 h-2.5 text-black fill-current ml-0.5" />
                              </div>
            ...
            <div className="flex justify-center gap-2 mt-6">
              {testimonials.map((_, idx) => <button key={idx} onClick={() => scrollToSlide(idx)} className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${current === idx ? 'bg-gradient-to-r from-primary to-secondary scale-125' : 'bg-gray-300 hover:bg-gray-400'}`} aria-label={`Go to testimonial ${idx + 1}`} />)}
            </div>
