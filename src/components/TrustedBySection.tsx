      ...
      <div className="container-custom">
        ...
        <div className="hidden md:block overflow-hidden">
          <div 
            ref={scrollRef} 
            style={{ minWidth: "200%" }} 
            className="flex items-center space-x-12 py-8 overflow-x-hidden w-full bg-neutral-50"
          >
            {/* First set of logos */}
            {logos.map(logo => (
              <div key={logo.id} className="flex-shrink-0 flex items-center justify-center bg-white p-6 rounded-md shadow-sm h-24 w-48">
            ...
            {logos.map(logo => (
              <div key={`duplicate-${logo.id}`} className="flex-shrink-0 flex items-center justify-center bg-white p-6 rounded-md shadow-sm h-24 w-48">
                <img src={logo.src} alt={logo.alt} className="max-h-14 max-w-full object-contain" />
              </div>
            ))}
          </div>
        ...
        <div className="md:hidden">
          <ScrollArea className="w-full whitespace-nowrap">
            <div className="flex space-x-6 py-6 px-4">
              {logos.map(logo => (
                <div key={logo.id} className="flex-shrink-0 flex items-center justify-center bg-white p-4 rounded-md shadow-sm h-20 w-40">
