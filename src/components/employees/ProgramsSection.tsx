...
const programs: Record<string, ProgramProps[]> = {
  fitness: [{
    ...
    benefits: ["Multiple fitness centers", "Flexible scheduling", "No long-term contracts", "Track your progress"]
  }, {
    ...
    benefits: ["Team building", "Motivating rewards", "Health tracking", "Friendly competition"]
  }],
  nutrition: [{
    ...
    benefits: ["Customized meal plans", "Expert nutritionist guidance", "Regular check-ins", "Adaptable to dietary restrictions"]
  }, {
    ...
    benefits: ["Better food choices at work", "Nutrition workshops", "Group cooking classes", "Healthy snack options"]
  }],
  mental: [{
    ...
    benefits: ["Meditation sessions", "Stress-reduction workshops", "Mental health resources", "Work-life balance guidance"]
  }, {
    ...
    benefits: ["Private coaching sessions", "Personalized strategies", "Progress tracking", "Ongoing support"]
  }]
};
  ...
  return <section className="section-padding">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {programs.fitness.map((program, index) => <Card key={`fitness-${index}`} className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
              
              <div className="p-6">
                <h3 className="text-xl font-bold mb-3">{program.title}</h3>
                <p className="mb-4 text-gray-600">{program.description}</p>
                <ul className="space-y-2">
                  {program.benefits.map((benefit, i) => <li key={i} className="flex items-center">
          ...
          {programs.nutrition.concat(programs.mental).map((program, index) => <Card key={`other-${index}`} className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow">
