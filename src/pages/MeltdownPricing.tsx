...
const MeltdownPricing = () => {
  return (
    <div className="min-h-screen bg-gradient-to-b from-yellow-50 via-gray-50 to-green-50 py-10">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 max-w-4xl mx-auto">
          {packages.map((pkg, i) => (
            <Card key={pkg.name} className={`rounded-2xl shadow transition-all border-2 ${pkg.color} border-opacity-40`}>
              ...
              <CardContent className="flex flex-col items-center">
                ...
                <Button variant="outline" size="lg" className={`mt-2 w-full ${pkg.buttonStyle}`}>
                  Select {pkg.name}
                </Button>
              </CardContent>
            </Card>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-100">
                ...
                {packages.map((pkg, i) => (
                  <TableHead key={pkg.name} className={`text-lg text-center bg-opacity-60`} style={{backgroundColor: i === 0 ? "#f3f4f6" : (i===1 ? "#fef9c3" : "#d1fae5")}}>
                    {pkg.name}
                  </TableHead>
                ))}
              </TableRow>
            ...
            <TableBody>
              {features.map((feature, fIdx) => (
                <TableRow key={feature.name} className={fIdx % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  {feature.values.map((hasFeature, pIdx) => (
                    <TableCell key={pIdx} className="text-center">
                      {feature.notes
                        ...
                        : hasFeature 
                        ? <CheckCircle2 className="mx-auto text-green-500 h-6 w-6" aria-label="Included" /> 
                        : <XCircle className="mx-auto text-gray-300 h-6 w-6" aria-label="Not included" />}
                    </TableCell>
                  ))}
                </TableRow>
              ))}
